...
/*   Updated: 2025/04/28 10:03:46 by kpedro           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	cylinder_cover_intersection(t_cylinder *cy, t_ray *ray)
{
	...
	t[0] = vector_dot(sub_vec(covers[0], ray->origin), cy->dir)
		/ vector_dot(ray->dir, cy->dir);
	t[1] = vector_dot(sub_vec(covers[1], ray->origin), cy->dir)
		/ vector_dot(ray->dir, cy->dir);
	...
	while (i < rt->nb.sphere)
	{
		if (intersect_box(sphere_box(&rt->sphere[i]), ray))
		{
			...
			if (t > 0 && t < closest.t)
			{
				...
				closest.index = i;
				closest.normal = sphere_normal(gen_point(t, ray),
						&rt->sphere[i]);
				closest.type = 's';
			}
		}
	...
	while (i < rt->nb.cylinder)
	{
		if (intersect_box(cylinder_box(&rt->cylinder[i]), ray))
		{
			...
			if (t > 0 && t < closest.t)
			{
				...
				closest.index = i;
				closest.normal = cylinder_normal(gen_point(t, ray),
						&rt->cylinder[i]);
				closest.type = 'c';
			}
		}
