...
/*   Updated: 2025/04/08 13:01:05 by kpedro           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rgb_color force(t_scene *rt, t_ray *ray, t_pair *obj)
{
	...
	color.red = rt->ambiente_light.intensity * (obj->color.red * rt->ambiente_light.color.red / 255.0);
	color.green = rt->ambiente_light.intensity * (obj->color.green * rt->ambiente_light.color.green / 255.0);
	color.blue = rt->ambiente_light.intensity * (obj->color.blue * rt->ambiente_light.color.blue / 255.0);
	...
	if (angle > 0)
	{
		color.red += ((rt->light.color.red / 255.0) * obj->color.red) * rt->light.brightness * angle;
		color.green += ((rt->light.color.green / 255.0) * obj->color.green) * rt->light.brightness * angle;
		color.blue += ((rt->light.color.blue / 255.0) * obj->color.blue) * rt->light.brightness * angle;
	}
